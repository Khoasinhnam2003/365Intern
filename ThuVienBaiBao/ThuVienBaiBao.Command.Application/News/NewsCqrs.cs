...
namespace ThuVienBaiBao.Command.Application.News;
...
public sealed class GetNewsQueryHandler : IRequestHandler<GetNewsQuery, IReadOnlyCollection<NewsSummaryDto>>
{
    ...
    public async Task<IReadOnlyCollection<NewsSummaryDto>> Handle(GetNewsQuery request, CancellationToken cancellationToken)
    {
        return await context.News
            ...
            .Select(news => new NewsSummaryDto(
                news.Id,
                news.Title,
                news.Slug,
                news.IsPublished,
...
public sealed class CreateNewsCommandHandler : IRequestHandler<CreateNewsCommand, NewsDetailDto>
{
    ...
    public CreateNewsCommandHandler(IApplicationDbContext context, IIntegrationEventPublisher integrationEventPublisher)
    {
        this.context = context;
    ...
    public async Task<NewsDetailDto> Handle(CreateNewsCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await GetNewsById(news.Id, cancellationToken) ?? throw new InvalidOperationException("News was not created.");

        await integrationEventPublisher.PublishNewsUpsertedAsync(
            new NewsUpsertedIntegrationEvent(
                result.Id,
                result.Title,
                result.Slug,
                result.Content,
                ...
                news.CreatedAt,
                news.UpdatedAt,
                result.Menus.Select(item => new RelatedMenuSnapshot(item.Id, item.Name, item.Slug)).ToList()),
            cancellationToken);
...
public sealed class UpdateNewsCommandHandler : IRequestHandler<UpdateNewsCommand, NewsDetailDto?>
{
    ...
    public UpdateNewsCommandHandler(IApplicationDbContext context, IIntegrationEventPublisher integrationEventPublisher)
    {
        this.context = context;
        this.integrationEventPublisher = integrationEventPublisher;
    }
    ...
    public async Task<NewsDetailDto?> Handle(UpdateNewsCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await GetNewsById(news.Id, cancellationToken);
...
public sealed class DeleteNewsCommandHandler : IRequestHandler<DeleteNewsCommand, bool>
{
    ...
    public DeleteNewsCommandHandler(IApplicationDbContext context, IIntegrationEventPublisher integrationEventPublisher)
    {
        this.context = context;
        this.integrationEventPublisher = integrationEventPublisher;
    }
    ...
    public async Task<bool> Handle(DeleteNewsCommand request, CancellationToken cancellationToken)
    {
        ...
        var relatedMenuIds = news.MenuNews.Select(link => link.MenuId).ToList();

        context.News.Remove(news);
        await context.SaveChangesAsync(cancellationToken);

        await integrationEventPublisher.PublishNewsDeletedAsync(
            new NewsDeletedIntegrationEvent(news.Id, relatedMenuIds),
            cancellationToken);
